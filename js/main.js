        ...
        if(http.readyState == 4 && http.status == 200) {
            ...
            if(data.data != '') {
                ...
                if(user_profile.image != null) {
                    user_image_div.innerHTML = '<img src="img/'+user_profile.image+'" /><div class="change-image-text">Change profile image</div>';
                } else {
                    user_image_div.innerHTML = '<img src="img/no-img.png" />';
                }
                username_div.innerHTML = user_profile.first_name + " " + user_profile.last_name;
            }
        }
    }
    http.send(null);
...
function getUsers(get_user = false) {
    user_list = [];
    var http = new XMLHttpRequest();
    http.open('GET', 'php/get_users.php', false); // IZMENA!!!
    http.onreadystatechange = function() {
        if(http.readyState == 4 && http.status == 200) {
            var data = JSON.parse(http.responseText);
            if(data.data != '') {
        ...
        if(feed.private && feed.author != user_id) {
            ...
            user_list.forEach(function(user) {
                if(user.id == feed.author) {
                    ...
                    feed_content += '<img src="img/' + user.image + '" /></a></div>';
                    feed_content += '<div class="feed-body"><p class="post-username"><b>' + user.name + '</b></p>';
                    feed_content += '<p class="post-time">' + feed.datetime + '</p>';
                }
            });
            if(feed.type == 'image') {
                feed_content += '<a href="image.html?id=' + feed.id + '"><img src="img/' + feed.content + '" /></a>';
            } else {
                feed_content += '<p class="post-content">' + feed.content + '</p>';
            }
...
function addStatus() {
    ...
    if(status_content.length > 140) {
        ...
        valid = false;
    } else if (status_content.length === 0) {
        errors.innerHTML += '<div class="error-msg">Status cannot be blank.</div>';
        valid = false;
    }
    if(valid) {
        var status = {
...
function addImage() {
    ...
    var private = false;
    if(confirm("Do you wish to upload this picture as private?")) {
        private = true;
    }
    if(image != undefined) {
